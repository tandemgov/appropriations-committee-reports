...
### Remaining
The Nemotron server was not reachable for this fix, so the header mapping and both signals are covered by unit tests but have not run end to end through `extract_house_hybrid`. Row-offset and merged-row errors of the kind the sign signal missed are not measured outside this report.
